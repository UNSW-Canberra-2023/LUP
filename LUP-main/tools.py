...
def get_gradient_values(model):
    ...
    """
    with torch.no_grad():
        gradients_org = [param.data.clone().detach() for param in model.parameters() if param.requires_grad]
        #learnable_layers = sum(p.requires_grad for p in model.parameters())
        #print("Number of learnable layers:", learnable_layers)
        gradient = torch.cat([torch.reshape(param.data.clone().detach() , (-1,)) for param in model.parameters() if param.requires_grad]).clone().detach()
    
    
    return gradient,gradients_org
    ...
    with torch.no_grad():
        for param in model.parameters():
                if param.requires_grad:
                    param.data.zero_()
                    param.data.copy_( torch.reshape(torch.narrow(gradient.data, 0, cur_pos, param.nelement()), param.size()))
...
def pairwise_distance_faster(gradients):
    device ='cpu' #gradients[0][0].device
    n = len(gradients)
